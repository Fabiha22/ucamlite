 ...
 * @author User
 */
public class ForgetT extends JFrame implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == SubmitButton) {
            if (isExistingTeacher(tf4.getText(), tf1.getText(), tf2.getText(), tf3.getText(), ComboBox.getSelectedItem().toString())) {
                dispose();
                ForgetBT pts = new ForgetBT();
                pts.setTitle("Login");
                pts.setVisible(true);
            } else {
    ...
    private boolean isExistingTeacher(String id, String name, String username, String address, String course) {
        boolean exists = false;
        try {
            java.sql.Connection connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/demo",
                    "root", "root");
            String query = "SELECT * FROM Teacher WHERE id=? AND name = ? AND username = ? AND address = ? AND coursetype = ?";
